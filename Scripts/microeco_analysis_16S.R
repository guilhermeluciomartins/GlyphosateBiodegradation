...
dataset$rarefy_samples(sample.size = 5150)
...
dataset$save_abund(dirpath = "taxa_abund_16S")
...
dataset$save_alphadiv(dirpath = "alpha_diversity_16S")
...
dataset$save_betadiv(dirpath = "beta_diversity_16S")
...
dev.print(tiff, "relative_abundance_facet_16S_phylum.tiff", compression = "lzw", res=600, height=15, width=20, units="cm")
...
#  save plot with 600 dpi resolution
dev.print(tiff, "heatmap_abundance_16S_genus.tiff", compression = "lzw", res=600, height=15, width=20, units="cm")
...
t1 <- trans_diff$new(dataset = dataset, method = "metastat", group = "Code", taxa_level = "Genus")
...
B_CF <- t1$plot_diff_abund(use_number = 1:10,
...
t1$plot_diff_abund(use_number = 1:15, col = c("orange2", "orange4"), add_sig = T, coord_flip = F, select_group = "CFN - CFN_D3")
#  save plot with 600 dpi resolution
...
B_NT <- t1$plot_diff_abund(use_number = 1:10,
...
bac_NT <- B_NT + rremove("xlab")
bac_NT


# Extract the legend labels from a ggplot object
leg <- get_legend(B_NT, position = "top")
as_ggplot(leg)
...
t1$plot_diff_abund(use_number = 1:20, col = c( "tomato2", "tomato4"), add_sig = T, coord_flip = F, select_group = "NTN - NTN_D3")



# Combine all diff abundance analysis in one figure
bac_diff <- ggarrange(bac_NF, bac_NT, bac_CF, 
                       labels = c("NF", "NT", "CF"),
                       common.legend = FALSE,
                       legend = "none",
                       align = "hv",
...
# Merge the two diff abundance figures
ggarrange(bac_diff, fung_diff, 
          labels = c("A", "B"),
          common.legend = FALSE,
          legend = "none",
          align = "hv",
...
dev.print(tiff, "Diff_abundance.tiff", compression = "lzw", res=800, height=25, width=35, units="cm")
...
#Df Variance       F Pr(>F)    
#B_glucosidase     1   123794  7.9714  0.001 ***
#Acid_phosphatase  1   187884 12.0983  0.001 ***
#pH                1   143591  9.2462  0.001 ***
#M.O.              1    45131  2.9061  0.019 *  
#P                 1    28356  1.8259  0.112    
#K                 1    11802  0.7599  0.536    
#C.CO2             1    98210  6.3239  0.001 ***
#Residual         37   574603     
...
RDA_16S <- t1$plot_ordination(plot_color = "Soil", plot_shape = "Dilution", col = c2, plot_type = c("point", "ellipse"), ellipse_chull_fill = F)
g1 <- RDA_16S + scale_shape_manual(values=c(15, 16, 17, 18)) + geom_text_repel()
g10 <- g1 + ggtitle("Bacterial community")
g10

#  save plot with 600 dpi resolution
dev.print(tiff, "RDA_genus_16S.tiff", compression = "lzw", res=600, height=12, width=20, units="cm")
...
#  save plot with 600 dpi resolution
dev.print(tiff, "RDA_genus2.tiff", compression = "lzw", res=800, height=15, width=28, units="cm")
